...
function fixtureHTML(f,extraClass=''){const home=isHome(f);const past=isPast(f);const maps=mapsUrl(f);const score=scoreOf(f);const result=ourResult(f);const middle=score?`<span class="score score-${result}">${scoreLabel(score)}</span>`:'<span class="vs">vs</span>';const note=score?`Uitslag ${scoreLabel(score)} · 22u · Volgens ATC-wedstrijdblad.`:`22u · Nog geen uitslag bij controle op ${formatChecked(clubData.checked)}.`;const route=home?'':routeLink(f,'Route');const venueExtra=`<small>${score?scoreLabel(score):'22u'}</small>`;return `<details class="fixture${past?' past':''}${extraClass}"><summary>${dateStamp(f)}<div><div class="fixture-teams"><span class="${home?'ours':''}">${escapeHTML(f.home)}</span>${middle}<span class="${!home?'ours':''}">${escapeHTML(f.away)}</span></div><div class="fixture-meta"><span class="${f.type==='beker'?'cup-tag':'competition'}">${f.type==='beker'?cupLabel():'COMPETITIE · 1A'}</span><span>·</span><span>${home?'THUIS':'UIT'}</span>${past?'<span class="played-tag">Gespeeld</span>':''}${route}</div></div><div class="venue">${escapeHTML(f.venue)}${venueExtra}</div><span class="fixture-toggle" aria-hidden="true">＋</span></summary><div class="fixture-detail"><p><strong>${escapeHTML(f.venue)}</strong><br>${escapeHTML(f.address)}<br>${note}</p><div class="fixture-links">${home?'':`<a class="route-link" href="${maps}" target="_blank" rel="noopener">${routeLabel('Route')}</a>`}<a href="${f.url}" target="_blank" rel="noopener">Wedstrijdblad ATC ↗</a></div></div></details>`}
function placeholderHTML(f){const past=isPast(f);const finales=String(f.note||'').toLowerCase().includes('finale');const copy=finales?'Finale volgens de ATC-kalender. Tegenstander volgt zodra ATC die bekendmaakt.':'Tegenstander volgt. ATC heeft deze bekerdag nog niet ingevuld.';return `<details class="fixture is-placeholder${past?' past':''}"><summary>${dateStamp(f)}<div><div class="fixture-teams"><span class="is-unknown">Beker</span></div><div class="fixture-meta"><span class="cup-tag">${cupLabel()}</span><span>·</span><span>Nog niet bekend</span></div></div><div class="venue">ATC-kalender</div><span class="fixture-toggle" aria-hidden="true">＋</span></summary><div class="fixture-detail"><p>${copy}</p><div class="fixture-links"><a href="https://www.atc-tafelvoetbal.be/calendar" target="_blank" rel="noopener">ATC-kalender ↗</a></div></div></details>`}
