...
function fixtureHTML(f,extraClass=''){const home=isHome(f);const past=isPast(f);const maps=mapsUrl(f);const score=scoreOf(f);const result=ourResult(f);const middle=score?`<span class="score score-${result}">${scoreLabel(score)}</span>`:'<span class="vs">vs</span>';const status=score?`Volgens ATC-wedstrijdblad.`:`Nog geen uitslag bij controle op ${formatChecked(clubData.checked)}.`;const route=home?'':routeLink(f,'Route');const venueExtra=`<small>${score?scoreLabel(score):'22u'}</small>`;const drive=driveFromMechelen(f);const facts=`<dl class="fixture-facts"><div><dt>Aanvang</dt><dd>22u</dd></div>${score?`<div><dt>Uitslag</dt><dd>${scoreLabel(score)}</dd></div>`:''}${drive?`<div><dt>Rit</dt><dd class="drive-note">${escapeHTML(drive)}</dd></div>`:''}</dl>`;return `<details class="fixture${past?' past':''}${extraClass}"><summary>${dateStamp(f)}<div><div class="fixture-teams"><span class="${home?'ours':''}">${escapeHTML(f.home)}</span>${middle}<span class="${!home?'ours':''}">${escapeHTML(f.away)}</span></div><div class="fixture-meta"><span class="${f.type==='beker'?'cup-tag':'competition'}">${f.type==='beker'?cupLabel():'COMPETITIE · 1A'}</span><span>·</span><span>${home?'THUIS':'UIT'}</span>${past?'<span class="played-tag">Gespeeld</span>':''}${route}</div></div><div class="venue">${escapeHTML(f.venue)}${venueExtra}</div><span class="fixture-toggle" aria-hidden="true">＋</span></summary><div class="fixture-detail"><div class="fixture-body"><div class="fixture-place"><p class="fixture-venue-name">${escapeHTML(f.venue)}</p><p class="fixture-address">${escapeHTML(f.address)}</p></div>${facts}<p class="fixture-status">${status}</p></div><div class="fixture-links">${home?'':`<a class="route-link" href="${maps}" target="_blank" rel="noopener">${routeLabel('Route')}</a>`}<a href="${f.url}" target="_blank" rel="noopener">Wedstrijdblad ATC ↗</a></div></div></details>`}
