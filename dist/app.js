...
function fixtureHTML(f,extraClass=''){const home=isHome(f);const past=isPast(f);const maps=mapsUrl(f);const score=scoreOf(f);const result=ourResult(f);const middle=score?`<span class="score score-${result}">${scoreLabel(score)}</span>`:'<span class="vs">vs</span>';const note=score?`Uitslag ${scoreLabel(score)} · 22u · Volgens ATC-wedstrijdblad.`:`22u · Nog geen uitslag bij controle op ${formatChecked(clubData.checked)}.`;const route=home?'':routeLink(f,'Route');const venueExtra=`<small>${score?scoreLabel(score):'22u'}</small>`;const drive=driveHint(f);const driveLine=drive?`<span class="drive-note">${escapeHTML(drive)} vanuit Mechelen</span>`:'';return `<details class="fixture${past?' past':''}${extraClass}"><summary>${dateStamp(f)}<div><div class="fixture-teams"><span class="${home?'ours':''}">${escapeHTML(f.home)}</span>${middle}<span class="${!home?'ours':''}">${escapeHTML(f.away)}</span></div><div class="fixture-meta"><span class="${f.type==='beker'?'cup-tag':'competition'}">${f.type==='beker'?cupLabel():'COMPETITIE · 1A'}</span><span>·</span><span>${home?'THUIS':'UIT'}</span>${past?'<span class="played-tag">Gespeeld</span>':''}${route}</div></div><div class="venue">${escapeHTML(f.venue)}${venueExtra}</div><span class="fixture-toggle" aria-hidden="true">＋</span></summary><div class="fixture-detail"><p><strong>${escapeHTML(f.venue)}</strong><br>${escapeHTML(f.address)}${driveLine?`<br>${driveLine}`:''}<br>${note}</p><div class="fixture-links">${home?'':`<a class="route-link" href="${maps}" target="_blank" rel="noopener">${routeLabel('Route')}</a>`}<a href="${f.url}" target="_blank" rel="noopener">Wedstrijdblad ATC ↗</a></div></div></details>`}
